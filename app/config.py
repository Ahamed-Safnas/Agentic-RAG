...
class Settings(BaseSettings):
    ...
    PORTKEY_FALLBACK_SLUG: str = "llm1"
    ...
    PORTKEY_PLANNER_CONFIG_ID: str | None = None
    PORTKEY_RESPONDER_CONFIG_ID: str | None = None
    PORTKEY_GUARDRAILS_CONFIG_ID: str | None = None
    PORTKEY_EVALS_CONFIG_ID: str | None = None
    PORTKEY_PLANNER_MODEL: str = "@rag/openai/gpt-oss-20b"
    PORTKEY_RESPONDER_MODEL: str = "@rag/openai/gpt-oss-20b"
    PORTKEY_GUARDRAILS_MODEL: str = "@rag/openai/gpt-oss-20b"
    PORTKEY_EVALS_MODEL: str = "@rag/openai/gpt-oss-20b"
    ...
    NEON_DB_URL: str
    # Maximum number of user and assistant messages retained per thread.
    MEMORY_MAX_MESSAGES: int = Field(default=20, ge=2)
    # Maximum character budget for the retained conversation window.
    MEMORY_MAX_CHARS: int = Field(default=12000, ge=1000)
    # Fail startup instead of using in-memory memory when Postgres is unavailable.
    MEMORY_REQUIRE_DURABLE_CHECKPOINTER: bool = False
    # Maximum number of connections used by the Postgres checkpoint pool.
    MEMORY_CHECKPOINT_POOL_MAX_SIZE: int = Field(default=20, ge=1)
    # Seconds to wait for a checkpoint-pool connection.
    MEMORY_CHECKPOINT_TIMEOUT_SECONDS: int = Field(default=10, ge=1)
    # Seconds before an idle checkpoint-pool connection is closed.
    MEMORY_CHECKPOINT_MAX_IDLE_SECONDS: int = Field(default=240, ge=1)
    # Maximum length accepted for a client-provided conversation thread ID.
    MEMORY_THREAD_ID_MAX_LENGTH: int = Field(default=128, ge=16)
    # Maximum number of past conversations returned to the UI sidebar.
    MEMORY_THREAD_LIST_LIMIT: int = Field(default=50, ge=1, le=200)
