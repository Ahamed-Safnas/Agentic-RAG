...
def load_thread(thread_id: str) -> None:
    """Load persisted messages and switch the UI to the selected thread."""
    try:
        base_url = os.getenv("BACKEND_URL", "http://localhost:8000")
        headers = {"Authorization": f"Bearer {os.getenv('RAG_API_KEY', '')}"}
        memory_url = f"{base_url}/memory/{quote(thread_id.strip(), safe='')}"
        memory_response = requests.get(memory_url, headers=headers, timeout=30)
        memory_response.raise_for_status()
        memory_data = memory_response.json()
        st.session_state.session_id = memory_data["thread_id"]
        st.session_state.messages = memory_data.get("messages", [])
        st.rerun()
    except Exception as e:
...
with st.sidebar:
    ...
    try:
        base_url = os.getenv("BACKEND_URL", "http://localhost:8000")
        headers = {"Authorization": f"Bearer {os.getenv('RAG_API_KEY', '')}"}
        threads_response = requests.get(f"{base_url}/memory", headers=headers, timeout=15)
        threads_response.raise_for_status()
        past_threads = threads_response.json().get("threads", [])
        if past_threads:
            for thread in past_threads:
                thread_id = thread["thread_id"]
                title = thread.get("title") or "New conversation"
                label = f"{title[:42]} ({thread.get('message_count', 0)})"
                if st.button(label, key=f"past_thread_{thread_id}", width="stretch"):
                    load_thread(thread_id)
        else:
            st.caption("No saved conversations yet.")
    except Exception as e:
        st.caption(f"Past conversations unavailable: {e}")

    with st.expander("Load by thread ID"):
        old_thread_id = st.text_input(
            "Thread ID",
            placeholder="Paste an old thread ID",
        )
        if st.button("📂 Load Conversation", width="stretch"):
            if not old_thread_id.strip():
